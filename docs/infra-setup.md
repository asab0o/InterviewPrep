...
- セットアップ・バックアップスクリプト：`infra/scripts/`
...
[AWS Amplify Hosting]
   ...
   └─ /api/<*> , /auth/<*>                  → rewrite（200プロキシ）で下記へ転送
      ...
      [SQLite ファイル（/opt/app/data/app.db）] on Lightsailの永続ディスク
         │  定期バックアップ（cron, sqlite3 .backup）
         ▼
      [S3バケット（バージョニング + Lifecycleで自動削除）]
```
...
#   backup_bucket_name         (グローバルで一意なS3バケット名)
...
terraform output -raw backup_uploader_access_key_id
terraform output -raw backup_uploader_secret_access_key
```

**重要**：`backup_uploader_secret_access_key` は一度きりの表示情報として厳重に扱い、パスワードマネージャー等に保管してください。
`terraform.tfstate` にも平文で残るため、`.gitignore`で除外されていることを確認し、可能ならリモートバックエンド（S3+DynamoDB）への移行も検討してください。

このコマンドで作成される主なリソース：
- Lightsail小型インスタンス（Ubuntu 22.04, micro_2_0プラン, 月$5）＋静的IP＋ファイアウォール
- SQLiteバックアップ用S3バケット（バージョニング・暗号化・Lifecycle自動削除つき）
- バックアップアップロード専用IAMユーザー（S3への書き込みのみ許可）
...
/opt/app/backups/     # バックアップ作業ディレクトリ
```
...
### 6.1 バックアップアップロード用IAM認証情報の設定

サーバー上のAWS CLIに、3章で発行した`backup_uploader`アクセスキー（Terraformの出力値）を設定します：

```bash
aws configure
# AWS Access Key ID: (terraform output -raw backup_uploader_access_key_id の値)
# AWS Secret Access Key: (terraform output -raw backup_uploader_secret_access_key の値)
# Default region name: ap-northeast-1
```
...
> パッケージマネージャは **pnpm**（npm/yarnは使わない）。AmplifyのビルドイメージにはNode.js 20系が
> 使われるためCorepackが同梱されており、追加インストール不要で有効化できる。
...
```bash
# スクリプトをサーバーに配置
scp -i ~/.ssh/lightsail_interview_prep infra/scripts/backup-sqlite-to-s3.sh \
  infra/scripts/restore-sqlite-from-s3.sh infra/scripts/setup-backup-cron.sh \
  ubuntu@<static-ip>:/opt/app/scripts/

ssh -i ~/.ssh/lightsail_interview_prep ubuntu@<static-ip>
chmod +x /opt/app/scripts/*.sh

# cron登録（毎日JST午前3時にバックアップ）
DB_PATH=/opt/app/data/app.db \
BACKUP_S3_BUCKET=$(terraform output -raw backup_bucket_name) \
/opt/app/scripts/setup-backup-cron.sh

# 手動で一度実行して動作確認
DB_PATH=/opt/app/data/app.db \
BACKUP_S3_BUCKET=$(terraform output -raw backup_bucket_name) \
bash /opt/app/scripts/backup-sqlite-to-s3.sh

# S3にアップロードされたか確認
aws s3 ls s3://$(terraform output -raw backup_bucket_name)/backups/
```

復元テスト（本番影響があるため、必要な時のみ実行。対話確認あり）：

```bash
DB_PATH=/opt/app/data/app.db \
BACKUP_S3_BUCKET=$(terraform output -raw backup_bucket_name) \
bash /opt/app/scripts/restore-sqlite-from-s3.sh
```
...
- [ ] SQLiteバックアップがS3に日次でアップロードされている
...
- S3バックアップ：数十MB〜数百MB程度のSQLiteファイル×Lifecycle保持日数分のみなので、月額数セント〜数十セント程度
