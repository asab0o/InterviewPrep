...
- 学習の一元管理
- 進捗のカテゴリー別可視化
を実現する。あわせて、AWS上へのデプロイ・認証構築を自らの手で行い、インフラ・クラウドの学習機会とする。
...
※上記のうち4〜6・8の一部は、`leetcode-interview-prep`リポジトリ側の運用効率化ツール（`/practice-start` `/practice-wrapup`コマンド、オンデバイス文字起こし。手順書：同リポジトリ`docs/practice-workflow-guide.md`）で先行して効率化済み。本アプリはこれをGUI化し、ダッシュボード（進捗の可視化）とフラッシュカード復習を加えるもの。アプリ完成までの間も日々の練習はこのツールで回す。
...
- シードデータは`leetcode-interview-prep/scripts/problems.json`をそのまま読み込んで投入する（7カテゴリー59問。付録Aと同一内容だが、こちらが今後更新される正本）
...
#### 5.4 GitHub連携【MVP】
- アプリ内でメモ・コードを編集
- 「Pushボタン」で該当リポジトリへ手動push（自動pushはしない＝誤送信防止のため意図的に手動）。バックエンドはAWS上で稼働するため、pushは**GitHub API（Octokit等）経由**で行う
...
- **競合回避ルール**：アプリが作成したファイルはアプリからのみ更新する。既存ファイル（Obsidian時代のもの、および`/practice-start`コマンドが作成したもの）はアプリから触らない。ローカルからのpushとアプリからのpushが同一ファイルに重ならないようにする
- **上書き防止チェック**：push前に同一パスのファイルが既に存在するか確認し、意図しない上書き（番号の偶然の重複等）であればユーザーに警告を表示してから続行する

#### 5.5 UMPIRE解説生成【MVP】
- 記録フォーム内に問題文を貼り付ける欄を用意し、Anthropic API（Claude）経由で既存のUMPIREプロンプトを実行（プロンプトの正本は`leetcode-interview-prep/docs/umpire-prompt.md`。付録Bに参考掲載）
...
- Obsidian時代の既存記録、および`/practice-start`コマンドで作成された記録（GitHubリポジトリ`problems/`配下のMarkdownファイル）を、開発段階でアプリのDBに初期データとして投入する
- GitHubへの再pushは不要（既にpush済み）。あくまでDBにAttemptレコードとして取り込み、ダッシュボードの網羅率・アタック数に過去分を反映させることが目的
- 取り込み方法：リポジトリのファイル一覧からファイル名（問題番号・問題名・再挑戦連番。kebab-case新形式・スペース区切り旧形式の両方に対応）とフォルダ名（カテゴリー）を解析してAttemptを生成する一回限りのスクリプトを想定
- ファイル名から取得できない情報（日付など）はファイルのコミット日時で代用、フレーズ等は空でよい

#### 5.8 記録の閲覧【MVP】
- **記録一覧**：過去のAttemptを日付順に一覧表示。カテゴリー・問題名での絞り込みが可能
- **記録詳細**：一覧から選択すると、その記録の全内容を閲覧できる
  - 日付・カテゴリー・問題名・コード・登録フレーズ（英日ペア）・UMPIRE解説
...
- **開発の進め方**：開発中はローカル（Vite dev server + Express + SQLite）で動作確認しながら実装し、機能がまとまった段階でAWSへデプロイする。デプロイ自体も学習項目として本人が手を動かして行う（Lightsailインスタンス起動、Docker化、Amplifyへのフロント配置、バックアップ設定等）
...
- **Problem**（NeetCode 150の問題マスタ。初期は7カテゴリー59問を投入。シードデータは`leetcode-interview-prep/scripts/problems.json`を正本とする）
  - id
...
- 外部API：
  - GitHub API（Octokit等、push用、MVP）
  - Anthropic API（MVP）：UMPIRE生成はSonnet系、フレーズの日本語訳サジェストはHaiku系を使用
  - YouTube Data API（フェーズ2）

## 9. 決定事項サマリー
- MVPスコープ：問題マスタ（NeetCode 150）／進捗ダッシュボード（網羅率＋総アタック数）／解答記録（オートコンプリート＋英日フレーズ登録＋Video URL）／UMPIRE解説生成（Problem単位で保存・再利用）／フラッシュカード復習（1日1回3問、JST基準）／GitHub手動push（GitHub API経由）／過去データの一括インポート（新旧命名対応）／記録閲覧（YouTube埋め込み）
...
- アプリ完成までの日々の練習は`leetcode-interview-prep`側の運用効率化ツールで継続（手順書：`docs/practice-workflow-guide.md`）
...
- ✅ 網羅率の精度：NeetCode 150問題マスタ（`scripts/problems.json`）＋オートコンプリートで表記ゆれを防止
...
- GitHub Personal Access Token または GitHub App（Octokit経由のAPI push用）
...
※このAppendixは参考掲載。実際にアプリが読み込む正本は`leetcode-interview-prep/scripts/problems.json`（運用効率化ツールと共通化済み）。
